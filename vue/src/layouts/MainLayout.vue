...
const { isAuthenticated, user, logout, deleteAccount, isAdmin } = useAuth()
    ...
    <header v-if="isAuthenticated" class="app-header">
      <div class="header-content">
        <nav class="navbar">
          ...
          <RouterLink v-if="isAdmin()" :to="{ name: 'admin-dashboard' }" class="nav-item admin-link">
            <Icon icon="mdi:shield-account" class="nav-icon" />
            <span>Admin</span>
          </RouterLink>
        </nav>
...
.navbar { display: flex; align-items: center; gap: 0; }
.nav-item { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.25rem; text-decoration: none; color: #64748b; font-weight: 500; transition: all 0.2s ease; border-bottom: 3px solid transparent; position: relative; }
...
.profile-section { margin-left: auto; display: flex; align-items: center; }
...
.dropdown-item:hover { background-color: #f8fafc; color: #6366f1; }
.dropdown-item.logout { color: #dc2626; border-top: 1px solid #e2e8f0; }
.dropdown-item.logout:hover { background-color: #fef2f2; color: #b91c1c; }
.dropdown-divider { height: 1px; background-color: #e2e8f0; margin: 0.5rem 0; }
.dropdown-item.delete { color: #dc2626; font-weight: 600; }
.dropdown-item.delete:hover:not(:disabled) { background-color: #fee2e2; color: #b91c1c; }
.dropdown-item.delete:disabled { opacity: 0.6; cursor: not-allowed; }
main { flex: 1; width: 100%; }
main.with-header { margin-top: 0; }
...
@media (max-width: 768px) { .header-content { padding: 0 1rem; } .nav-item { padding: 0.75rem 0.85rem; font-size: 0.9rem; } .nav-item span { display: none; } .profile-avatar, .header-avatar { width: 40px; height: 40px; font-size: 1rem; } }
</style>
...
async function getAllClubs() {
    // Added: WHERE status != 'suspended'
    // I also added 'status' to the SELECT just in case your frontend needs it!
...
async function getClubById(id) {
    // Added: AND status != 'suspended' to prevent direct link access
    const [rows] = await pool.query(
        `SELECT id, title, description, status 
         FROM clubs 
...
async function joinClub(profileId, clubId) {
    ...
    const [exists] = await pool.query(
        "SELECT id FROM club_members WHERE profile_id = ? AND club_id = ?",
        [profileId, clubId],
    );
    if (exists.length > 0) return { already: true };

    const [result] = await pool.query(
        "INSERT INTO club_members (profile_id, role, club_id) VALUES (?, ?, ?)",
        [profileId, "member", clubId],
    );
...
async function approveJoinRequest(requestId, clubId) {
    const [req] = await pool.query(
        "SELECT profile_id FROM club_join_requests WHERE id = ? AND club_id = ?",
        [requestId, clubId],
    );
...
async function declineJoinRequest(requestId, clubId) {
    await pool.query(
        'UPDATE club_join_requests SET status = "declined" WHERE id = ? AND club_id = ?',
        [requestId, clubId],
    );
    return { success: true };
}
...
async function removeMember(memberId, clubId) {
    const [result] = await pool.query(
        "DELETE FROM club_members WHERE id = ? AND club_id = ?",
        [memberId, clubId],
    );
...
async function changeMemberRole(memberId, clubId, newRole) {
    const [result] = await pool.query(
        "UPDATE club_members SET role = ? WHERE id = ? AND club_id = ?",
        [newRole, memberId, clubId],
    );
    return { success: result.affectedRows > 0 };
}
...
async function updateClub(clubId, title, description) {
    const [result] = await pool.query(
        "UPDATE clubs SET title = ?, description = ? WHERE id = ?",

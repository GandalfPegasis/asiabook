...
router.post('/:id/request-join', authMiddleware, async (req, res) => {
    try {
        ...
        if (result.already) return res.status(200).json({ success: true, message: 'Request already sent' });
        res.json({ success: true, id: result.id });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Server error' });
...
router.post('/:id/requests/:requestId/approve', authMiddleware, async (req, res) => {
    try {
        const clubId = req.params.id;
...
router.post('/:id/requests/:requestId/decline', authMiddleware, async (req, res) => {
    try {
        const clubId = req.params.id;
        const requestId = req.params.requestId;
...
router.patch('/:id/members/:memberId/role', authMiddleware, async (req, res) => {
    try {
        const clubId = req.params.id;
        const memberId = req.params.memberId;
        const { role } = req.body;
        ...
        const result = await clubService.changeMemberRole(memberId, clubId, role);
        res.json(result);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Server error' });
...
router.post('/:id/events', authMiddleware, async (req, res) => {
    try {
        ...
        const event = await clubService.createClubEvent(clubId, title, description, event_date, location);
        res.json(event);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Server error' });
...
router.put('/:id/events/:eventId', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await clubService.updateClubEvent(eventId, clubId, title, description, event_date, location);
        res.json(result);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Server error' });
